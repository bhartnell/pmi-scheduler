...
| BUG-7 | [[ARCHIVED] superseded ACLS lab days still sit live on the Oct 5 and Oct 6 dates](https://app.notion.com/p/3dc5cd1cbc9b81e9a1fde0fef9afce8e) | High | Code patch | Not yet verified | 2026-09-14 | 2026-09-15 | 2026-09-28 | Both rows already carried is_archived=true (set by BUG-6's migration). 2 of the 4 surfaces named in the ticket didn't filter it: /api/calendar/feed.ics and syncLabStationAssignment (station-instructors route). Fixed both (.eq('is_archived', false) / an is_archived check before the sync). 0 calendar events + 0 station instructors existed for either row, so latent gap, not a live incident. lab-days/[id], grading-context, timer-display, and /api/calendar/unified left as documented gaps (ID-scoped or already accidentally correct). Manually synced 2026-09-15 — no NOTION_API_KEY in this container to run export-bug-log.js. |
