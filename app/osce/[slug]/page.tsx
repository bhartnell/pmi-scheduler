...
export default function OsceDynamicSignup() {
  const params = useParams();
  const slug = params.slug as string;

  const [event, setEvent] = useState<OsceEventPublic | null>(null);
  ...
  const fetchEvent = async () => {
    setPageLoading(true);
    try {
      const res = await fetch(`/api/osce/public/${slug}`);
      if (!res.ok) {
        setNotFound(true);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/osce/public/${slug}/register`, {
        method: 'POST',
  ...
  // Build info cards from unique dates
  const uniqueDates = [...new Set(blocks.map(b => b.date))].sort();
  const getBlockTimesForDate = (date: string) => {
    const dateBlocks = blocks.filter(b => b.date === date);
    if (dateBlocks.length === 0) return '';
    const earliest = dateBlocks.reduce((min, b) => b.start_time < min ? b.start_time : min, dateBlocks[0].start_time);
    const latest = dateBlocks.reduce((max, b) => b.end_time > max ? b.end_time : max, dateBlocks[0].end_time);
    ...
    return (
      <>
        ...
        <div className="osce-signup">
          ...
          <div style={{ maxWidth: 720, margin: '3rem auto', textAlign: 'center', color: '#8A8A8A' }}>
            <span className="osce-loading-spinner" style={{ display: 'inline-block' }} />
          </div>
        </div>
      </>
    );
        ...
        <div className="osce-signup">
          <header className="osce-hero">
            <div className="osce-hero-inner">
              ...
              <p className="osce-hero-subtitle">This OSCE event page doesn&apos;t exist or has been archived.</p>
            </div>
          </header>
          <div className="osce-footer">
            <p>Questions? Contact <a href="mailto:bhartnell@pmi.edu">Benjamin Hartnell</a> &mdash; Lead Paramedic Instructor &amp; Clinical Director</p>
          </div>
        </div>
      </>
    );
        ...
        <div className="osce-signup">
          <header className="osce-hero">
            <div className="osce-hero-inner">
              <p className="osce-institution">Pima Medical Institute &mdash; Paramedic Program</p>
              <h1>{event.title}</h1>
              {event.subtitle && <p className="osce-hero-subtitle">{event.subtitle}</p>}
            </div>
          </header>
          <section className="osce-form-section">
            <div className="osce-form-container" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '1.5rem', color: '#0F1F3D', marginBottom: '1rem' }}>Registration Not Yet Open</h2>
              <p style={{ color: '#5A5A5A', lineHeight: 1.6 }}>
                Registration for this event hasn&apos;t opened yet. Please check back soon or contact us for more information.
              </p>
            </div>
          </section>
          <div className="osce-footer">
    ...
    return (
      <>
        ...
        <div className="osce-signup">
          ...
          <section className="osce-form-section">
            <div className="osce-form-container" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: '1.5rem', color: '#0F1F3D', marginBottom: '1rem' }}>Registration Closed</h2>
              <p style={{ color: '#5A5A5A', lineHeight: 1.6 }}>
                Registration for this event has closed. If you need to make changes to an existing registration, please contact us directly.
              </p>
            </div>
          </section>
          <div className="osce-footer">
  ...
  // Parse location into name/detail
    ...
    <>
      ...
      <div className="osce-signup">
        <header className="osce-hero">
          <div className="osce-hero-inner">
            <p className="osce-institution">Pima Medical Institute &mdash; Paramedic Program</p>
            <h1>{event.title}<br />Evaluator Registration</h1>
            <p className="osce-hero-subtitle">{event.description || `Join our Medical Directors and faculty as an evaluator for the ${event.title}.`}</p>
          </div>
        </header>

        <div className="osce-info-strip">
          {uniqueDates.map((date, i) => (
            <div key={date} className="osce-info-card">
              <div className="osce-label">Day {i + 1}</div>
              <div className="osce-value">{formatShortDate(date)}</div>
              <div className="osce-sub">{getBlockTimesForDate(date)}</div>
            </div>
          ))}
          {event.location && (
            <div className="osce-info-card">
              <div className="osce-label">Location</div>
              <div className="osce-value">{locationParts[0]}</div>
              {locationParts[1] && <div className="osce-sub">{locationParts[1]}</div>}
            </div>
          )}
        </div>
          ...
          <div className="osce-form-container">
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="osce-avail-group">
                ...
                <p className="osce-avail-hint">Each time block is capped at {blocks[0]?.max_observers || event.max_observers_per_block || 4} outside evaluators. Full blocks are locked.</p>
                ...
                {triedSubmit && selectedBlocks.size === 0 && !allBlocksFull && (
...
// Extracted CSS as a constant to keep the component clean
const osceStyles = `
...
.osce-info-strip { max-width: 720px; margin: -1.5rem auto 0; padding: 0 2rem; position: relative; z-index: 2; display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem; opacity: 0; animation: osceSlideUp 0.6s ease forwards 0.45s; }
