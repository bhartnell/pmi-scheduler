      ...
      <main className={`max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6${showRosterPrint ? ' print:hidden' : ''}`}>
        ...
        <div className="space-y-6 lg:space-y-0 lg:grid lg:gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
          <div className="lg:col-start-1 lg:row-start-1">
            <StationCards stations={labDay.stations} stationSkillDocs={stationSkillDocs} stationDocs={stationDocs} stationSkillSheetIds={stationSkillSheetIds} stationNremtCodes={stationNremtCodes} stationScenarioTitles={nremtScenarioTitles} canSelectScenario={!!userRole && hasMinRole(userRole, 'lead_instructor')} isAdvCertMegacode={!!labDay.is_adv_cert_testing || /megacode/i.test(labDay.section_label || '')} certCourse={labDay.cert_course} calendarAvailability={calendarAvailability} labDayId={labDayId as string} getStationTitle={getStationTitle} onEditStation={(station) => setEditingStation(station)} onOpenRoleModal={(station) => setRoleModalStation(station)} onOpenScenarioPicker={(station, code) => setScenarioPickerState({ station, code })} />
          </div>
