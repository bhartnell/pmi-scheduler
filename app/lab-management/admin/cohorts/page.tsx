...
export default function CohortManagementPage() {
  ...
  const [editTrackClinical, setEditTrackClinical] = useState(false);
  ...
  const startEdit = (cohort: Cohort) => {
    ...
    setEditTrackClinical(cohort.track_clinical_hours ?? false);
  };
    ...
    try {
      ...
      const res = await fetch(`/api/lab-management/cohorts/${cohortId}`, {
        ...
        body: JSON.stringify({
          ...
          track_clinical_hours: editTrackClinical,
        }),
      });
                    ...
                    {editingId === cohort.id ? (
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                        ...
                        <div className="flex flex-wrap gap-2 flex-1 items-center">
                          ...
                          />
                          <label className="flex items-center gap-1.5 px-2 py-1 rounded text-sm cursor-pointer select-none whitespace-nowrap">
                            <input
                              type="checkbox"
                              checked={editTrackClinical}
                              onChange={(e) => setEditTrackClinical(e.target.checked)}
                              className="rounded border-gray-300 dark:border-gray-600 text-emerald-600"
                            />
                            <Activity className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                            <span className="text-gray-700 dark:text-gray-300">Track Clinical</span>
                          </label>
                        </div>
                        <div className="flex gap-2">
                          <button
