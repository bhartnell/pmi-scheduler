...
type TabKey = 'observers' | 'schedule' | 'agencies' | 'blocks' | 'students' | 'results' | 'settings';
...
export default function OsceEventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tabs: { key: TabKey; label: string; icon: React.ReactNode }[] = [
    ...
    { key: 'results', label: 'Results', icon: <ClipboardCheck className="w-4 h-4" /> },
...
interface ResultsEvaluatorResult {
...
interface ResultsAssessmentResult {
  id: string;
  student_name: string;
  scenario: string;
  slot_number: number;
  ...
  evaluators: ResultsEvaluatorResult[];
  evaluator_count: number;
  submitted_count: number;
}

const RESULTS_FACTOR_LABELS: Record<string, string> = {
...
const RESULTS_ORAL_LABELS: Record<string, string> = {
...
function ResultsRatingBadge({ value }: { value: string | null }) {
...
function ResultsReadinessLabel({ value }: { value: string | null }) {
...
function ResultsTab({ eventId }: { eventId: string }) {
  const [results, setResults] = useState<ResultsAssessmentResult[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/osce/results?event_id=${eventId}`);
        const data = await res.json();
  ...
  if (results.length === 0) {
    return <div className="p-8 text-center text-gray-500 dark:text-gray-400">No results yet for this event.</div>;
  }
                            ...
                            <div key={i} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center gap-3 text-sm">
                                  ...
                                  <ResultsReadinessLabel value={ev.readiness} />
                                  {ev.submitted_at && (
                              ...
                              {/* SNHD Factors */}
                              <div className="grid grid-cols-8 max-md:grid-cols-4 gap-2 mb-3">
                                {Object.entries(RESULTS_FACTOR_LABELS).map(([key, label]) => (
                                  <div key={key} className="text-center">
                                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mb-1 truncate" title={label}>{label}</p>
                                    <ResultsRatingBadge value={ev.factors[key]?.rating ?? null} />
                                  </div>
                                ))}
                              </div>
                              ...
                              <div className="grid grid-cols-5 gap-2 mb-3">
                                {Object.entries(RESULTS_ORAL_LABELS).map(([key, label]) => (
                                  <div key={key} className="text-center">
                                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mb-1 truncate" title={label}>{label}</p>
                                    <ResultsRatingBadge value={ev.oral[key as keyof typeof ev.oral] ?? null} />
                                  </div>
                                ))}
                              </div>
