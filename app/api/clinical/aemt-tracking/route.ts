...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get AEMT tracking data for these students
    ...
    console.error('Error fetching AEMT tracking:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch AEMT tracking' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error saving AEMT tracking:', error);
    return NextResponse.json({ success: false, error: 'Failed to save AEMT tracking' }, { status: 500 });
  }
}
