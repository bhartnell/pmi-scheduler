...
export default function AEMTTrackingPage() {
  ...
  const cohortLabel = selectedCohortDetails
    ? `AEMT Group ${selectedCohortDetails.cohort_number}`
    : 'AEMT';

  // Export configuration
  const exportConfig: ExportConfig = {
    title: 'AEMT Student Tracking Roster',
    subtitle: selectedCohortDetails ? cohortLabel : undefined,
    filename: `aemt-tracking-${cohortLabel.replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}`,
    columns: [
