...
export default function StudentEvaluationDetailPage() {
  ...
  const fetchEvaluation = async () => {
    try {
      // Try student endpoint first, then instructor endpoint
      ...
      if (data.success && data.evaluations?.length > 0) {
        ...
        const eval_ = data.evaluations.find((e: any) => e.id === evaluationId);
        if (eval_) {
          // Need to fetch steps separately
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8 text-center max-w-sm">
          ...
          <Link href="/student/skill-sheets" className="text-blue-600 dark:text-blue-400 hover:underline">
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 px-4 py-3">
        <nav className="max-w-2xl mx-auto flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <Link href="/student" className="hover:text-blue-600"><Home className="w-4 h-4" /></Link>
          <ChevronRight className="w-3 h-3" />
          <Link href="/student/skill-sheets" className="hover:text-blue-600">Skill Sheets</Link>
          <ChevronRight className="w-3 h-3" />
