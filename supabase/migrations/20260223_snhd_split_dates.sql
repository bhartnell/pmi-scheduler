...
--   1. snhd_field_docs_submitted_at  - Field internship documentation submission date
--   2. snhd_course_completion_submitted_at - SNHD course completion record submission date
...
SET
  snhd_field_docs_submitted_at = snhd_submitted_date,
  snhd_course_completion_submitted_at = snhd_submitted_date
WHERE snhd_submitted_date IS NOT NULL
  AND snhd_field_docs_submitted_at IS NULL
  AND snhd_course_completion_submitted_at IS NULL;

-- Add indexes for common queries
CREATE INDEX IF NOT EXISTS idx_student_internships_snhd_field_doc
  ON student_internships(snhd_field_docs_submitted_at);

CREATE INDEX IF NOT EXISTS idx_student_internships_snhd_course_completion
  ON student_internships(snhd_course_completion_submitted_at);
