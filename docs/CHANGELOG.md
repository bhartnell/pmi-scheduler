...
- `PENDING` | ACLS Day-2 flexible buffer relabeled to name the decision point (final schedule-rebuild piece). The 1300–1400 block is now `ACLS LAB · Extra Megacode Practice if needed — else start Testing` (was "Megacode Practice (if needed) — flexible buffer") — honestly describes the run-time choice (skip extra practice → start testing at 1300). Re-ran `scripts/rebuild-acls-blocks.js` (30 blocks; prior set backed up to `g14_acls_blocks_pre_rebuild3_20260615.json`; G5 preserved; stations untouched). Concludes the ACLS schedule-block rebuild.
