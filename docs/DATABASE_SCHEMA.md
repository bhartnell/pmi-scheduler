...
> Last updated: 2026-09-01 -- same plan, two more slices: (1) Tier 0 follow-up (migration `20260901_tier0b_new_backup_tables_enable_rls.sql`) enabled RLS on 11 more `_backup_*` tables created since 08-28 by unrelated fixes; (2) CRITICAL item (migration `20260901_tier3_revoke_anon_execute_admin_rpcs.sql`) revoked anon/authenticated `EXECUTE` on 7 SECURITY DEFINER functions with zero client-side callers -- `get_all_users`, `promote_student_to_program`, `delete_station_admin`, `update_library_item_status`, `create_notification` (both overloads), and 2 calendar-link triggers; `service_role`'s separate grant is untouched. No column/table shape changes. Still open: Tier 2 (~49 live-table RLS policies) + Tier 3 hardening, and a newly-flagged schema-drift finding -- `moi_check_key`/`moi_set_station`/`moi_station_progress` (plus `moi_teacher`/`moi_session`/`moi_submissions` tables) exist live in production with no corresponding migration anywhere in the repo; not documented here pending Ben confirming what they are.
