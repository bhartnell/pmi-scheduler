...
> Last updated: 2026-09-01 -- Tier 3 critical item (migration `20260901_tier3_revoke_anon_execute_admin_rpcs.sql`, PR #84): revoked the implicit `PUBLIC` `EXECUTE` grant (inherited by `anon`/`authenticated`) on 7 SECURITY DEFINER functions with zero anon/authenticated callers anywhere in the app -- `get_all_users()`, `create_notification(...)` (both overloads), `update_library_item_status()`, `delete_station_admin()`, `promote_student_to_program()`, `pmi_link_block_on_lab_day_insert()`, `pmi_link_lab_day_on_block_publish()`. `service_role`'s separate grant is untouched. No column/table shape changes.
> Last updated: 2026-09-01 -- Tier 3 remainder (migration `20260901_critical_revoke_anon_execute_security_definer.sql`, PR #86): revoked the `PUBLIC` `EXECUTE` grant on the other 6 flagged SECURITY DEFINER functions -- `is_superadmin`, `is_access_admin`, `has_ops_access`, `is_inventory_admin`, `is_print_operator` (referenced inside `authenticated`-role RLS policy USING/WITH CHECK clauses on ~35 tables, so `authenticated`'s own explicit grant was re-added after the blanket PUBLIC revoke -- only `anon` access is actually closed for these 5) and `has_pmi_ops_role` (confirmed via `pg_policies` to have zero policy references -- revoked fully, both roles). `moi_check_key`/`moi_set_station`/`moi_station_progress` intentionally left anon-executable (shared-key kiosk auth model, not a Postgres-role gate) and flagged to Ben -- not defined in any tracked migration, schema drift outside this pass's scope. No column/table shape changes.
