...
> Last updated: 2026-09-01 -- CRITICAL item complete: revoked anon/authenticated `EXECUTE` on the 7 orphaned SECURITY DEFINER functions with zero live callers (`20260901_revoke_anon_execute_orphaned_security_definer_fns.sql`, PR #84) and revoked anon-only `EXECUTE` on 5 of the 6 RLS-predicate helper functions plus anon+authenticated on `has_pmi_ops_role` (`20260901_tier1b_revoke_anon_execute_rls_predicate_helpers.sql` -- tracks a fix a concurrent session already applied directly to prod; `authenticated` deliberately kept on `is_superadmin`/`has_ops_access`/`is_access_admin`/`is_inventory_admin`/`is_print_operator` pending the Tier 2 table-by-table RLS audit of the ~20 access/inventory/print/library tables whose policies call them). `moi_check_key`/`moi_set_station`/`moi_station_progress` intentionally left anon-executable (shared-key kiosk auth model, not a Postgres-role gate) and flagged to Ben -- not defined in any tracked migration, schema drift outside this pass's scope. No column/table shape changes.
