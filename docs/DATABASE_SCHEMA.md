...
> Last updated: 2026-09-01 -- same plan, closed the CRITICAL item + a Tier 0 follow-up (four parallel Code sessions worked this Notion queue item concurrently today; reconciled here): (1) Tier 0 follow-up (migration `20260901_tier0b_new_backup_tables_enable_rls.sql`) enabled RLS on 11 more `_backup_*` tables created since 08-28; (2) all 14 originally-flagged anon-executable SECURITY DEFINER functions now closed -- `get_all_users`, `promote_student_to_program`, `delete_station_admin`, `update_library_item_status`, `create_notification` (both overloads), 2 calendar-link triggers (migrations `20260901_tier3_revoke_anon_execute_admin_rpcs.sql` + `20260901_fix_tier3_revoke_used_ineffective_role_list.sql` -- the first one's `REVOKE ... FROM anon, authenticated` was a no-op against the implicit PUBLIC grant these functions actually had; the fix migration uses the correct `REVOKE ... FROM PUBLIC` form and is the one that matches what's live), plus `is_superadmin`/`is_access_admin`/`is_inventory_admin`/`is_print_operator`/`has_ops_access`/`has_pmi_ops_role` (migration `20260901_critical_revoke_anon_execute_security_definer.sql`, `authenticated` retained where those gate live RLS policies). `service_role`'s separate grant is untouched throughout. No column/table shape changes. Still open: Tier 2 (~49 live-table RLS policies) + Tier 3 hardening (function search_path, security-definer views, leaked-password protection), and a schema-drift finding -- `moi_check_key`/`moi_set_station`/`moi_station_progress` (plus `moi_teacher`/`moi_session`/`moi_submissions` tables) exist live in production with no corresponding migration anywhere in the repo; not documented here pending Ben confirming what they are.
...
**ACLS certification template (Phase 2 of AHA HUB + ACLS REPEATABLE, 2026-07-13):** 7 new `category='certification', cert_course='acls'` rows seeded directly (data-only, no migration — `created_by='claude-code-task-handoff-queue:aha-hub-acls-repeatable-phase2'`), mirroring G14's live ACLS structure (cohort `8577fdc3-...`) read-only, without modifying any of G14's own `lab_days`/`lab_stations`: Day 1 §2 "Learning — Cardiac Arrest" (4 scenario stations), §3 "Pre-Arrest (Combined)" (4 scenario), §4 "BLS" (2 skills), §5 "Airway" (1 skills); Day 2 §2 "Megacode Practice — Set 1" (4 scenario), §3 "Megacode Testing" (`is_adv_cert_testing=true`, `lab_mode='individual_testing'`, 4 unlinked scenario-slot stations), §4 "Megacode Practice — Set 2" (4 scenario). G14's Day-1 §1 and Day-2's already-`[ARCHIVED]` §1 were deliberately excluded — §1 on Day 1 (unlabeled, still linked via `source_template_id` to the original pre-migration `lab_day_templates` stub `b0b27a24-...`) reads as the same kind of pre-"sections" leftover that Day 2's §1 was already explicitly archived for, just never archived itself; flagged for Ben, not touched. The 2 original `category='certification'` stub rows (`b0b27a24-...` Day 1, `3b12db4b-...` Day 2, both still `cert_course=NULL`) are left as-is (orphaned, unused by the new template) rather than repurposed, since both already carry a `source_template_id` link into that same stale/archived G14 data. **Data-quality finding surfaced, not fixed on G14:** Day-1 §2's live station labels read `CASE_34/36/39/40` but the scenarios actually linked are `CASE_45/48/47/49`, and Day-2 §3's live `MEGACODE_TEST_1..4` labels don't reliably match their linked scenario either — the new template uses the real linked-scenario data (or, for §3, no fixed link at all, since the testing day draws from the pool at grading time) rather than propagating either mismatch. Verified live via read-only `SELECT` before and after writing; G14's own 9 `lab_days` rows confirmed byte-for-byte unchanged post-write. Phase 3 dry-run validated the resulting template resolves to 7 `create` actions (correct station counts) for cohort `856bcf1d-...` (PM 15.0, next real non-G14 cohort) via the generator's exact SQL logic (no `.env.local` in this session to invoke `generateAhaCourseForCohort` directly — verified by replicating its query, not by running the TS function; flagged for a follow-up session with real app credentials to do a true end-to-end dry-run through the UI/API).
...
**Check Constraints:**
- `google_calendar_events_source_type_check`: `((source_type = ANY (ARRAY['station_assignment'::text, 'lab_day_role'::text, 'shift_signup'::text, 'site_visit'::text, 'osce_block'::text, 'osce_instructor'::text, 'schedule_block'::text, 'schedule_block_series'::text, 'poll_meeting'::text, 'lvfr_assignment'::text, 'general_lab'::text, 'pals_all_day'::text, 'pals_day'::text])))` — `lvfr_assignment` added by `20260706_widen_gcal_source_type_lvfr.sql`; `general_lab` added by `20260708_gcal_general_lab_source_type.sql` (the general-lab-default event, `source_id = lab_day_id`, for full-time paramedic instructors); `pals_all_day` added by `20260715_gcal_source_type_pals_all_day.sql` (**deprecated** all-day per-section model); `pals_day` added by `20260716_gcal_source_type_pals_day.sql` — the **current** PALS model: ONE SCHEDULED 08:30-16:30 block per REAL PALS course date, `source_id = '{cohort_id}:{date}'`, one per instructor per real PALS day (`lib/pals-all-day-sync.ts` → `syncPalsDayEvents`). The `syncPalsDayEvents` reconcile deletes all `pals_all_day` rows; `syncGeneralLabDefaults` now removes `general_lab` on `cert_course='pals'` + `is_archived` lab days (never creates them there). **Rule: any new `source_type` value in code MUST ship with a constraint-widening migration in the same commit.**
