...
> Last updated: 2026-09-01 -- RLS hardening Tier 2 (migration `20260901_tier2_live_tables_enable_rls.sql`): enabled RLS (no policies yet -- service-role-only, same mechanism as Tier 0/1) on the 49 remaining LIVE tables the Supabase advisors flagged `rls_disabled_in_public`, several holding student PII/grade-adjacent data (`assessment_rubrics`, `rubric_criteria`, `peer_evaluations`, `skill_signoffs`, `student_communications`, `student_program_enrollments`, `grade_access_log`, `medications`, plus 41 others -- full list in the migration file). Confirmed via live `has_table_privilege`/`SET LOCAL ROLE anon` test that anon previously had full SELECT/INSERT/UPDATE/DELETE at the table-grant level with RLS disabled (direct PostgREST/anon-key exposure bypassing the app entirely) and now returns zero rows post-fix; confirmed via grep that no client-side/anon-key Supabase usage in `app/**/*.tsx` touches any of these 49 tables (all app access is server-side via `getSupabaseAdmin()`/service_role, which bypasses RLS), so this closes the exposure with no app-behavior change. Per-role policies (Tier 3) remain future work. No column/table shape changes. See Task Handoff Queue `[SECURITY - advisors, NEEDS BEN]`.
